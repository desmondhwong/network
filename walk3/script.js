...
let popupVisible = false; // Start with false if the graph should be hidden initially
...
function getRandomAngle(x, y) {
    // Calculate the angle from the current position back to the origin
    const angleToOrigin = Math.atan2(-y, -x);  // Origin at (0,0), adjust signs to point back

    // Get a random angle that incorporates bias towards the origin
    // 'random() * 360deg' becomes 'random() * 2 * Math.PI' for radians, adjust for bias
    const randomBias = (Math.random() - 0.5);  // Center around 0 to allow negative and positive adjustments
    const adjustedAngle = angleToOrigin + randomBias * Math.PI;  // Reduce the range of randomness for stronger bias
...
function drawRandomWalkFromSteps(steps) {
    ...
    for (let i = 0; i < steps.length; i++) {
        // Calculate angle back to the origin from the current position
        const angleToOrigin = Math.atan2(-y + canvas.height / 2, -x + canvas.width / 2);
        const randomBias = (Math.random() - 0.5);  // Center around 0 to allow negative and positive adjustments
        const adjustedAngle = angleToOrigin + randomBias * Math.PI;  // Reduce the range of randomness for stronger bias

        const newX = x + unitToPixels(Math.cos(adjustedAngle));
        const newY = y + unitToPixels(Math.sin(adjustedAngle));
...
resetButton.addEventListener('click', () => {
    ...
    highestIntersectionElement.textContent = '0';  // Correct the reset value
    updatePermutationNumber();
});


// Function to toggle the visibility of the popup panel
function togglePopup() {
    popupVisible = !popupVisible;
    popupPanel.style.display = popupVisible ? 'block' : 'none';
    if (popupVisible && generations.length > 0) {
        drawHorizontalBarGraph(); // Redraw graph when shown
    }
}
...
document.getElementById('togglePopupButton').addEventListener('click', togglePopup);

// Ensure the graph updates dynamically and is formatted correctly
function drawHorizontalBarGraph() {
    const intersectionCounts = generations.reduce((counts, g) => {
        counts[g.intersections] = (counts[g.intersections] || 0) + 1;
        return counts;
    }, {});

    const totalPermutations = generations.length;
    const sortedIntersections = Object.keys(intersectionCounts).sort((a, b) => a - b);

    const barGraphContainer = document.getElementById('barGraphContainer');
    barGraphContainer.innerHTML = ''; // Clear previous bars

    sortedIntersections.forEach((yValue) => {
        const count = intersectionCounts[yValue];
        const percentage = (count / totalPermutations) * 100;
        const bar = document.createElement('div');
        bar.className = 'bar';
        bar.style.width = `${percentage}%`;  // Ensure this is being applied correctly
    
        const label = document.createElement('div');
        label.className = 'bar-label';
        label.textContent = `${yValue}: ${count} (${percentage.toFixed(2)}%)`;  // Updated label format with count and percentage
...
// Update the bar graph every time a new generation is generated
...
// Update the highest intersection count and bar graph when a new generation is generated
function updateHighestIntersections() {
    const maxIntersections = Math.max(...generations.map(g => g.intersections), 0);
    highestIntersectionsIndices = generations.reduce((indices, g, index) => {
        if (g.intersections === maxIntersections) {
    ...
    // Update the display to show the highest number only once
    document.getElementById('highestIntersections').innerText = maxIntersections;
